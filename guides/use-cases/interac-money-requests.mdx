...
If the payer does not approve before `expires_at`, Blaaiz sends a collection webhook with `transaction_status` set to `EXPIRED` and `message` set to `Transaction Expired`. No funds move. Use this webhook to close the pending request in your system. See [Expired transactions](/guides/webhooks/collection/cad#expired-transactions).
