...
<Note>
Transaction list and detail responses use `external_reference` for collections and payouts in all currencies.
CAD collection webhooks use `interac_reference` for the same saved collection reference.
</Note>
...
| `interac_reference`                  | The saved Interac reference. A string, or `null` when no reference is available. |
| `merchant_reference`                  | Always `null`. An Interac collection has no reference from you.                       |
