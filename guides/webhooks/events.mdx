...
| `auto_deposit_email.active` | Interac auto-deposit registration activated | `collection_url`, with `payout_url` fallback | [Interac auto-deposit email](/guides/webhooks/interac-auto-deposit-email) |
| `auto_deposit_email.blocked` | Interac auto-deposit registration blocked or an active registration removed | `collection_url`, with `payout_url` fallback | [Interac auto-deposit email](/guides/webhooks/interac-auto-deposit-email) |
...
| `crypto.deposit.successful` | Crypto deposit credited to a business crypto wallet. The payload identifies the customer when a customer wallet address is funded. | `collection_url` | [Crypto API](/guides/crypto-api) |
...
| `merchant.kyc.session.completed` | A Merchant KYC verification session finished review. `data.result` is `APPROVED` or `REJECTED` | `kyc_url` | [Merchant KYC](/compliance/webhooks) |
| `merchant.kyc.session.expired` | A Merchant KYC verification session passed its deadline before it completed | `kyc_url` | [Merchant KYC](/compliance/webhooks) |

<Note>
Payout and crypto payout events use your `payout_url`. Merchant KYC events use your `kyc_url`. Interac auto-deposit registration events use `collection_url`, or `payout_url` when `collection_url` is null. Crypto deposit events and the other listed events use `collection_url`.
</Note>

<Note>
Merchant KYC events use their own payload shape and their own retry schedule. See [Merchant KYC webhooks](/compliance/webhooks).
</Note>

Interac auto-deposit registration events use a flat payload with `type`, `email`, `status`, `message`, and `event_id`.
See [registration status webhooks](/guides/webhooks/interac-auto-deposit-email) for the exact payload.
