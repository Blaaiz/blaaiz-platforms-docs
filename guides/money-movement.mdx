...
sequenceDiagram
    ...
    You->>Blaaiz: POST /payout (amount, recipient)
    Blaaiz->>Blaaiz: Calculate payout fee
    Blaaiz->>Wallet: Debit full amount
    Blaaiz->>Recipient: Send (amount - fee)
    Blaaiz->>Webhook: payout.completed webhook
```

Payout fees are **inclusive** — they are deducted from the amount withdrawn from your wallet:

1. You specify the payout `amount` (this is the "you send" amount).
2. Blaaiz calculates the applicable **payout fee** (which may include a fixed fee, percentage fee, or both).
3. The **total amount (including fees)** is deducted from your wallet.
4. The **recipient receives** the amount minus fees.
5. A `payout.completed` webhook is sent to your `payout_url`.

**Example**: You send 1,000 USD to a recipient with a 5 USD payout fee.
...
<Note>
If your wallet balance is insufficient to cover the payout amount (which includes fees), the payout will fail.
</Note>

### Recipient gets mode

When you specify the amount as "recipient gets" instead of "you send", the calculation reverses:

1. You specify how much the recipient should receive.
2. Blaaiz calculates the fees and adds them on top.
3. The total (recipient amount + fees) is deducted from your wallet.

**Example**: You want the recipient to get exactly 1,000 USD, with a 5 USD payout fee.

| Step | Amount |
|------|--------|
| Recipient gets | 1,000 USD |
| Payout fee | 5 USD |
...
| Flow | Fee handling | What your wallet sees |
|------|-------------|----------------------|
| **Collection** (VIBAN, card, open banking) | Fees deducted from incoming amount | Credited: amount minus fees |
| **Payout** (you send) | Fees deducted from payout amount | Debited: full amount you specified |
| **Payout** (recipient gets) | Fees added on top of recipient amount | Debited: recipient amount + fees |

## Related

- [Fee breakdown](/api-reference/fees/get-breakdown) — preview fees before a transaction.
- [Initiate collection](/api-reference/collection/create) — collect funds via card or open banking.
