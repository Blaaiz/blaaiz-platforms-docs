...
Use the returned access token as a Bearer token on every external API request. The `scope` parameter should be a space-separated list of the scopes assigned to your credentials. See [Authentication — Scopes](/guides/authentication#scopes) for the full list.
