...
Use the returned access token as a Bearer token on every external API request. The required scope for external business API access is `business-api-services`.
