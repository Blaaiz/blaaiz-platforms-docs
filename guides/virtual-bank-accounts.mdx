...
<Note>
**Customer type restrictions by currency.** NGN Virtual Bank Accounts work for both `type=individual` and `type=business` customers — businesses must complete the [KYB flow](/guides/kyb/overview) (beneficial owners + KYB documents + `/submit`) before the customer is `VERIFIED`. **GBP, EUR, and USD currently support `type=individual` only.** `type=business` for those currencies is not yet available; KYB is the foundation for that future expansion.
</Note>
