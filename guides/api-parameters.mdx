...
**For business customers**, `id_type` is **no longer required** — businesses identify themselves via `registration_number` + `incorporation_country` instead. The legacy `id_type` and `id_number` fields remain accepted on the request shape for back-compat; if you do send `id_type` on a business customer, the only valid value is `certificate_of_incorporation`. The actual certificate file goes into the KYB document collection (`type=CERTIFICATE_OF_INCORPORATION`), not through the File API. See [Business customer KYB](/guides/kyb/overview).
