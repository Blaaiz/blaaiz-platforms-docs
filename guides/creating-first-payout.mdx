...
<Note>
  The customer's `country` must match the identity document provided for KYC. Also note that only `NON-VERIFIED` customers can be fully updated. If a customer is already `VERIFIED`, only fields that are currently `null` can be updated.
</Note>
...
<Tabs>
  <Tab title="All Currencies">
    ...
    **Customer must be verified**
    ...
    Every payout must be linked to a verified customer. If your customer's identity documents have not been uploaded and approved, the payout will be rejected. Make sure you have completed Step 1 and Step 2 above before attempting a payout.
