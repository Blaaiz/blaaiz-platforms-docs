...
- **Business customers**: `id_type` is no longer required at the customer level — businesses identify via `registration_number` + `incorporation_country`. If you do send `id_type` for back-compat, only **Certificate of Incorporation** (`certificate_of_incorporation`) is valid. The actual certificate file goes into the [KYB document collection](/guides/kyb/onboarding), not through the File API.
