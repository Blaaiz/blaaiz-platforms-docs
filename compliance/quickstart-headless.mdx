...
description: Upload identity documents and proof of address from your own server, then submit the session for review.
---
...
A headless session asks for `DOCUMENTS`, `PROOF_OF_ADDRESS`, or both. If you need a live selfie, use the [Blaaiz-hosted quickstart](/compliance/quickstart-hosted) instead. Read [Merchant KYC overview](/compliance/overview) first for the session model.
...
| `id_doc_type` | One of `PASSPORT`, `ID_CARD`, `DRIVERS`, `RESIDENCE_PERMIT`, `UTILITY_BILL`, `SELFIE`. `UTILITY_BILL` satisfies the proof-of-address step. Every other type satisfies the documents step. |
...
When the requirement set includes `PROOF_OF_ADDRESS`, upload the proof of address the same way. Repeat steps 2 to 4 with `id_doc_type` set to `UTILITY_BILL`. A photo and a PDF are both accepted, so send `image/jpeg`, `image/png`, `image/webp`, or `application/pdf`.

```json
{
  "filename": "utility-bill.pdf",
  "content_type": "application/pdf",
  "id_doc_type": "UTILITY_BILL",
  "country": "NGA",
  "file_name": "b1e8d3f704_utility bill.pdf"
}
```

On a proof of address, `country` means the country of the address **on the document**. It is not the nationality of the person, and it is not the country of the identity document. A person can hold a passport of one country and a utility bill of another.
