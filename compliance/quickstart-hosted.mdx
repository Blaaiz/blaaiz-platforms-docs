...
The capture page also collects the proof of address when the session asks for it. The page shows one step for each requirement in the set, and nothing more.
...
Ask for the full set — `DOCUMENTS`, `SELFIE`, and `FACE_MATCH` — and set `fulfilment_mode` to `BLAAIZ_HOSTED`. To collect the proof of address on the same page, add `PROOF_OF_ADDRESS` to `requirements`.
...
Send the `capture_url` to the person by email, SMS, or in your own app. The page asks the person for each step of the set in turn.
...
The capture-link endpoint fails when the session is terminal, and when `fulfilment_mode` is not `BLAAIZ_HOSTED`. Every `BLAAIZ_HOSTED` session can take a capture link, including a session with no selfie step.
