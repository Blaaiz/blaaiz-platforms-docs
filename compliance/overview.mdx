...
| --- | --- | --- |
| `BLAAIZ_HOSTED` | A Blaaiz capture page, at `verify.blaaiz.com` in production. The page shows one step for each requirement in the set: the documents, the proof of address, and the selfie. | Issue a capture link and send it to the person. |
...
| `["DOCUMENTS", "SELFIE", "FACE_MATCH", "PROOF_OF_ADDRESS"]` | `SUMSUB_HOSTED` | Yes |
| `["DOCUMENTS", "SELFIE", "FACE_MATCH", "PROOF_OF_ADDRESS"]` | `BLAAIZ_HOSTED` | No |
| `["DOCUMENTS", "PROOF_OF_ADDRESS"]` | `SUMSUB_HOSTED` | Yes |
| `["DOCUMENTS", "PROOF_OF_ADDRESS"]` | `BLAAIZ_HOSTED` | No |
| `["DOCUMENTS", "PROOF_OF_ADDRESS"]` | `HEADLESS` | No |
| `["PROOF_OF_ADDRESS"]` | `SUMSUB_HOSTED` | Yes |
| `["PROOF_OF_ADDRESS"]` | `BLAAIZ_HOSTED` | No |
| `["PROOF_OF_ADDRESS"]` | `HEADLESS` | No |

Every set that includes `PROOF_OF_ADDRESS` is available in all three modes, except the set that also includes `SELFIE`. That set needs Blaaiz or the verification partner to capture the selfie, so it is never headless.
